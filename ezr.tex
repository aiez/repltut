...
% Solid disc marker: \disc{1} (black) or \disc[red]{2} assasd
...
As evidence we present 400 lines of EZR.py, a Python toolkit
...
Tested on the 120+ tabular SE optimization tasks in the
MOOT repository, these tiny tools perform as well as or
better than state-of-the-art explanation tools
(SHAP, LIME), the SMAC3 optimizer, and SVM-based
text-mining filters (FASTREAD), while running 500$\times$
faster than SMAC3, using orders of magnitude less labelled
data, and building trees from fewer than ten variables even
when thousands are available.
...
\textsf{pip install ezr}; example data at \textsf{github.com/timm/moot}.
}
...
\midrule
\S\ref{sec:famous} &
Classification, clustering, and seeded clustering are usually
...
vector machines) or 
LLM-grade embeddings~\cite{yu2018total,yu18}. &
...
\begin{center}
{\fontsize{7.5}{8}\selectfont
\begin{tabularx}{.8\textwidth}{@{} l l Y @{}}
\textbf{Axis} & \textbf{\IT{}} & \textbf{Heavy comparator} \\
\midrule
Lines of code (toolkit only)& 400                       & \texttt{sklearn} $>$200k (toolkit; excludes Python stdlib both sides)~\cite{martinez2021} \\
...
Labels to reach 85--95\% optimum & $<100$               & SMAC3, SHAP, ReliefF: 1{,}000s~\cite{ganguly2026lowgodatalightse,Amiraliminimaldata} \\
...
Pedagogical complexity      & A few classroom sessions     & \texttt{sklearn} API spans semester-length courses \\
...
pedagogical complexity (one session vs.\ semester course), and
failure mode (user-patchable vs.\ upstream-queued). On each
axis \IT{} sits one or more orders of magnitude below the
...
encoders or kernel suppport vector machines~\cite{Yu2018}. 
\IT{}'s complementary Bayes filter
(see \S\ref{sec:textmine}) one the other hand, achieves comparable recall using the
...
normally distributed. So the sum of three uniforms, centered and
...
The second is {empirical}: that modern
...
In the 1990s, LS with all its variants (GSAT~\cite{gsat},In the 1990s, LS with all its variants (GSAT~\cite{gsat}, WalkSAT~\cite{walksat}, GRASP~\cite{grasp}, Iterated LS~\cite{ils})
...
unlabeled example; (c)~which is then labeled
...
to some research query.
 Reading
every candidate takes weeks or months~\cite{yu2017feature,yu2018}. 
...
and normalized CNB (red) on Yu's four SLR corpora. Warm-start
...
300--600 labels to reach the same recall.
...
complementary Bayes converges faster (40 labels to recover
108 of the 132 relevant papers) but caps at a lower
...
subset; in this paper, results are reported across all 124
tasks of Table~\ref{mootdata}, spanning configuration tuning,
performance prediction, defect prediction, test selection,
cost estimation, and text mining. Second, prior text-mining
...
work~\cite{yu2018total} reported only recall. External
replications by Rayegan et al.~\cite{Amiraliminimaldata}
and Ganguly et al.~\cite{ganguly2026lowgodatalightse} use
independent methodologies and reach compatible conclusions,
which strengthens (but does not eliminate) this threat.
...
To mitigate from this threat,
our code base is small (400 lines) and
publicly available
(\url{https://github.com/timm/ezr}). Two external
groups~\cite{Amiraliminimaldata,ganguly2026lowgodatalightse}
have independently re-run \IT{}'s algorithms against their
own pipelines and reached compatible conclusions, which
bounds the probability that our results are an artifact of
encoding error.
...
The decisive evidence that the surrogate is adequate is in
Table~\ref{tab:opt} itself: every \textsf{win} score is
computed on the {\em true} \textsf{disty} of the row the
search finally selected, not on the surrogate. SA reaches a
mean win of 98--99 out of 100 across 20 randomly selected
MOOT tasks and lands on 100 for 12 of them. A surrogate
that drives the search to the reference optimum on this
fraction of tasks is doing its job. 
